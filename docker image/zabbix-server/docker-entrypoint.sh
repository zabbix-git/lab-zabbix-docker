...
check_variables_mysql() {
    ...
    DB_SERVER_ZBX_USER=${MYSQL_USER:-"SERVERUSER_BD"}
    DB_SERVER_ZBX_PASS=${MYSQL_PASSWORD:-"PASS_SERVERUSER_BD"}
...
update_zbx_config() {
    ...
    update_config_var $ZBX_CONFIG "LogFile" "${ZABBIX_ETC_DIR}/zabbix_server.log"
    ...
    if [ "${ZBX_JAVAGATEWAY_ENABLE}" == "true" ]; then
        ...
        update_config_var $ZBX_CONFIG "StartJavaPollers" "${ZBX_STARTJAVAPOLLERS:-"0"}"
    else
